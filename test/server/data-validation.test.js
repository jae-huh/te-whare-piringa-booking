...
test('"ok" message returned if booking does not overlap with any other bookings', t => {
  const booking = Object.create(exampleBooking)
...
test('"ok" message returned if booking starts when another ends and otherwise does\'t overlap', t => {
  const booking = Object.create(exampleBooking)
  const expected = 'ok'
  const actual = validate.checkBookingForOverlap(booking, exampleBookings)
  t.is(actual, expected, 'function should return a string')
...
test('message returned if booking starts when another booking starts', t => {
  const booking = Object.create(exampleBooking)
...
test('message returned if booking overlaps the start date of another booking', t => {
  const booking = Object.create(exampleBooking)
...
test('message returned if booking overlaps the end date of another booking', t => {
  const booking = Object.create(exampleBooking)
...
test('message returned if booking overlaps all of another booking', t => {
  const booking = Object.create(exampleBooking)
  booking.startDate = new Date(tomorrow.getFullYear(), tomorrow.getMonth(), tomorrow.getDate(), 11)
  booking.endDate = new Date(tomorrow.getFullYear(), tomorrow.getMonth(), tomorrow.getDate(), 21)
  const expected = 'Your request overlaps with another booking'
